...
test('check() returns true on HTTP 200 response', function () {
    ...
    $this->cmd->shouldReceive('remote')
        ->with("echo \"DISK:\$(df -h '/var/www/test' | tail -1 | awk '{print \$5}' | sed 's/%//')\" && echo \"DISK_AVAIL:\$(df -h '/var/www/test' | tail -1 | awk '{print \$4}')\" && echo \"MEM:\$(free | grep Mem | awk '{print int(\$3/\$2 * 100)}')\"")
        ->andReturn("DISK:45\nDISK_AVAIL:12G\nMEM:60");
    $this->cmd->shouldReceive('info')
        ->with('Disk usage: 45%')
        ->once();
